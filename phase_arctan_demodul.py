...
def main():
    ...
    for i in range(num_frames):
        ...
        amplitude = np.abs(result.frame[0])  # shape: (num_points,)
        max_bin_idx = np.argmax(amplitude)

        # Get phase at the tracked bin
        tracked_iq = result.frame[0][max_bin_idx]  # complex scalar
        phase = np.angle(tracked_iq)
